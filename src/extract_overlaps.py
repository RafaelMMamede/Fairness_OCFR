...
def positive_contributions(importance_map,importance_th = .8):
    normalized_importance = (importance_map-np.min(importance_map))/(np.max(importance_map)-np.min(importance_map)) 

    return  np.where(normalized_importance>importance_th,1,0)
  
def negative_contributions(importance_map,importance_th = .2):
    normalized_importance = (importance_map-np.min(importance_map))/(np.max(importance_map)-np.min(importance_map)) 
